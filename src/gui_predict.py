...
class FruitVegClassifierGUI:
    def __init__(self, root):
        ...
        self.model_path = os.path.join(os.path.dirname(__file__), "model.pth")  # 默认模型路径
    ...
    def create_widgets(self):
        ...
        # 创建图像标签，设置为可调整大小
        self.image_label = ttk.Label(self.image_frame, text="请上传图像",
                                     anchor="center", padding="20")
        ...
        # 上传按钮
        self.upload_button = ttk.Button(left_frame, text="上传图像",
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="上传图像", command=self.upload_image, accelerator="Ctrl+U")
        file_menu.add_command(label="加载模型", command=self.load_model, accelerator="Ctrl+M")
        ...
        view_menu.add_command(label="查看评估结果", command=self.show_evaluation_results)
    ...
    def predict_image(self, file_path):
        ...
        try:
            ...
            self.root.after(0, messagebox.showerror, "预测错误", f"预测过程中出错: {str(e)}")
    ...
    def show_about(self):
        ...
        ttk.Label(about_window, text="水果和蔬菜分类器", font=("Arial", 16, "bold")).pack(pady=20)
